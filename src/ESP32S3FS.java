...
/*
  ...
  into an SPIFFS partition image and upload it to an ESP32S3 MCU
 ...
 */
public class ESP32S3FS implements Tool {
  Editor editor;
  ...
  public String getMenuTitle() {
    return "ESP32S3 Sketch Data Upload";
  }
  ...
  private void createAndUpload(){
    ...
    if(isNetwork){
      ...
      if(esptool.getAbsolutePath().endsWith(".py"))
        sysExec(new String[]{pythonCmd, esptool.getAbsolutePath(), "--chip", "esp32-s3", "--baud", uploadSpeed, "--port", serialPort, "--before", "default_reset", "--after", "hard_reset", "write_flash", "-z", "--flash_mode", flashMode, "--flash_freq", flashFreq, "--flash_size", "detect", ""+spiStart, imagePath});
      else
        sysExec(new String[]{esptool.getAbsolutePath(), "--chip", "esp32-s3", "--baud", uploadSpeed, "--port", serialPort, "--before", "default_reset", "--after", "hard_reset", "write_flash", "-z", "--flash_mode", flashMode, "--flash_freq", flashFreq, "--flash_size", "detect", ""+spiStart, imagePath});
    }
  }
